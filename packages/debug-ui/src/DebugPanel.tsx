...
export interface SetLullState {
  setState: string;
  wavesSpawned: number;
  currentSetWaves: number;
...
export interface Toggles {
  ...
  showSetWaves: boolean;
  showBackgroundWaves: boolean;
  showEnergyField: boolean;
  showFoamSamples: boolean;
  showFoamZones: boolean;
  showPlayer: boolean;
  showAIPlayer: boolean;
  showFoamOptionA: boolean;
  showFoamOptionB: boolean;
  showFoamOptionC: boolean;
...
export interface DebugPanelProps {
  ...
  onPlayerConfigChange?: (key: keyof PlayerConfig, value: number) => void;
  aiMode?: string;
  onAIModeChange?: () => void;
  cameraConfig?: CameraConfig;
  onCameraConfigChange?: (key: keyof CameraConfig, value: number) => void;
}
  ...
  return (
    <div className="debug-panel">
      ...
      <Section title="View Layers">
        ...
        {toggles.showPlayer && toggles.showAIPlayer && onAIModeChange && (
          <Toggle
            label="AI Mode"
            checked={true}
      ...
      {cameraConfig && onCameraConfigChange && (
        <Section title="Camera (3D)">
          <Slider
            label="Height"
            tooltip="Camera height above the scene (units)."
            value={cameraConfig.height}
            min={5}
            max={100}
            step={1}
            onChange={(v) => onCameraConfigChange('height', v)}
          />
          <Slider
            label="Distance"
            tooltip="Camera offset from center. Negative = toward shore, positive = toward ocean."
            value={cameraConfig.distance}
            min={-50}
            max={50}
            step={1}
            onChange={(v) => onCameraConfigChange('distance', v)}
          />
          <Slider
            label="Tilt"
            tooltip="Camera angle. -90 = straight down, 0 = horizontal."
            value={cameraConfig.tilt}
            min={-90}
            max={0}
            step={5}
            onChange={(v) => onCameraConfigChange('tilt', v)}
          />
        </Section>
      )}
      ...
      {toggles.showPlayer && playerConfig && onPlayerConfigChange && (
...
function Toggle({ label, checked, onChange, hotkey, text = null }: ToggleProps) {
  return (
...
function Select({ label, value, options, onChange, hotkey }: SelectProps) {
  const cycleValue = () => {
...
function Slider({ label, value, min, max, step = 1, onChange, suffix = '', tooltip }: SliderProps) {
...
function CircularProgress({ progress, size = 20, strokeWidth = 3, color }: CircularProgressProps) {
...
interface CountdownReadOnlyProps {
  label: string;
  remaining: number;
  total: number;
  progress: number;
...
function CountdownReadOnly({ label, remaining, total, progress, color }: CountdownReadOnlyProps) {
  return (
