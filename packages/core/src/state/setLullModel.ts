...
export function getNextWaveTime(config, randomFn = Math.random) {
  return (
    config.swellPeriod + randomInRange(-config.periodVariation, config.periodVariation, randomFn)
  );
}
...
export function initializeLull(state, config, randomFn = Math.random, gameTime = 0) {
  const lullDuration =
    config.lullDuration + randomInRange(-config.lullVariation, config.lullVariation, randomFn);
  const currentSetWaves = Math.floor(
    randomInRange(config.lullWavesPerSet[0], config.lullWavesPerSet[1] + 1, randomFn)
  );
...
export function initializeSet(state, config, randomFn = Math.random, gameTime = 0) {
  const currentSetWaves = Math.floor(
    randomInRange(config.wavesPerSet[0], config.wavesPerSet[1] + 1, randomFn)
  );
...
export function shouldSpawnWave(state, gameTime) {
  ...
  return timeSinceLastWave >= state.nextWaveTime && state.wavesSpawned < state.currentSetWaves;
}
 ...
 */
export function updateSetLullState(
  state,
  gameTime,
