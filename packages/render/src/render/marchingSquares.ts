    ...
    for (const seg of row.segments) {
      ...
      for (let gx = startGridX; gx <= endGridX && gx < gridW; gx++) {
        ...
        if (distFromCenter <= originalHalfWidth) {
          ...
          const haloProgress =
            (distFromCenter - originalHalfWidth) / (expandedWidth / 2 - originalHalfWidth);
      ...
      for (const seg of row.segments) {
        ...
        for (let gx = startGridX; gx <= endGridX && gx < gridW; gx++) {
          ...
          if (distFromCenter <= originalHalfWidth) {
            ...
            const haloProgress =
              (distFromCenter - originalHalfWidth) / (expandedHalfWidth - originalHalfWidth);
  ...
  while (iterations++ < maxIterations) {
    ...
    switch (exitEdge) {
      case 0:
        nextCy = cy - 1;
        nextFromEdge = 2;
        break; // Exit top, enter from bottom
      case 1:
        nextCx = cx + 1;
        nextFromEdge = 3;
        break; // Exit right, enter from left
      case 2:
        nextCy = cy + 1;
        nextFromEdge = 0;
        break; // Exit bottom, enter from top
      case 3:
        nextCx = cx - 1;
        nextFromEdge = 1;
        break; // Exit left, enter from right
    }
    ...
    for (let cx = 0; cx < w - 1; cx++) {
      ...
      const segmentTable = {
        ...
        5: [
          [3, 0],
          [1, 2],
        ], // BL, TR above (saddle - ambiguous, pick one)
        6: [[2, 0]], // BR, TR above
        ...
        10: [
          [0, 1],
          [2, 3],
        ], // BR, TL above (saddle - ambiguous, pick one)
 ...
 */
export function renderContourDebug(
  ctx,
  foamRows,
  canvasW,
  canvasH,
 ...
 * @param {number} options.blurPasses - Blur passes (default 2)
 */
export function renderMultiContour(
  ctx,
  foamRows,
  canvasW,
  canvasH,
 ...
 */
export function renderMultiContourOptionA(
  ctx,
  foamRows,
  canvasW,
  canvasH,
 ...
 */
export function renderMultiContourOptionB(
  ctx,
  foamRows,
  canvasW,
  canvasH,
  ...
  const { grid, blurPasses } = buildIntensityGridOptionB(
    foamRows,
    gridW,
    gridH,
    canvasW,
 ...
 */
export function renderMultiContourOptionC(
  ctx,
  foamRows,
  canvasW,
  canvasH,
 ...
 */
export function renderMultiContourFromGrid(
  ctx,
  foamGrid,
  gridW,
  gridH,
...
export function renderMultiContourOptionAFromGrid(
  ctx,
  foamGrid,
  gridW,
  gridH,
...
export function renderMultiContourOptionBFromGrid(
  ctx,
  foamGrid,
  gridW,
  gridH,
...
export function renderMultiContourOptionCFromGrid(
  ctx,
  foamGrid,
  gridW,
  gridH,
