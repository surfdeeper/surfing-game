...
describe('AI Player Integration', () => {
    it('logs real game dimensions', () => {
        ...
        console.log(`Peak X: ${DEFAULT_BATHYMETRY.peakX} = ${DEFAULT_BATHYMETRY.peakX * CANVAS_WIDTH}px`);
    ...
    it('AI paddles from spawn to target position', () => {
        ...
        const targetY = oceanTop + (oceanBottom - oceanTop) * aiState.config.targetLineup;
        console.log(`Target position: (${peakX}, ${targetY})`);
        console.log(`Target lineup config: ${aiState.config.targetLineup}`);
        ...
        for (let frame = 0; frame < frames; frame++) {
            ...
            if (frame - lastLogFrame >= 120) {
                ...
                console.log(`Frame ${frame}: pos=(${Math.round(player.x)},${Math.round(player.y)}) dx=${Math.round(dx)} dy=${Math.round(dy)} input=${JSON.stringify(input)}`);
            }
        }

        // Calculate where the AI should end up (center of target zone)
        const defaultY = oceanTop + (oceanBottom - oceanTop) * ((aiState.config.minProgress + aiState.config.maxProgress) / 2);
    ...
    it('AI catches wave when foam reaches them at target position', () => {
        ...
        const targetY = oceanTop + (oceanBottom - oceanTop) * ((aiState.config.minProgress + aiState.config.maxProgress) / 2);
        ...
        for (let attempt = 0; attempt < 10; attempt++) {
            ...
            updateAIPlayer(player, aiState, world, 1/60, CANVAS_WIDTH, CANVAS_HEIGHT, oceanTop, oceanBottom, TRAVEL_DURATION);
    ...
    it('AI reaches target and catches foam', () => {
        ...
        const targetY = oceanTop + (oceanBottom - oceanTop) * 0.70;
    ...
    it('verifies coordinate system', () => {
        ...
        console.log(`Expert zone: ${aiState.config.minProgress}-${aiState.config.maxProgress} progress`);
        console.log(`Expert zone Y: ${aiState.config.minProgress * oceanBottom}-${aiState.config.maxProgress * oceanBottom}px`);
        console.log(`Peak starts at progress ${DEFAULT_BATHYMETRY.peakStartProgress} = Y=${DEFAULT_BATHYMETRY.peakStartProgress * oceanBottom}`);

        // Verify expert zone starts at the peak
        const peakStartProgress = DEFAULT_BATHYMETRY.peakStartProgress; // 0.55

        console.log('');
        console.log(`Expert min (${aiState.config.minProgress}) >= peak start (${peakStartProgress}): ${aiState.config.minProgress >= peakStartProgress}`);
