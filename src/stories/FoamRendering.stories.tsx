  ...
  for (const wave of waves) {
    ...
    for (let y = oceanTop; y <= waveY; y += foamYSpacing) {
      ...
      for (let i = 0; i <= numXSamples; i++) {
        ...
        const depth =
          i < numXSamples ? getDepth(normalizedX, DEFAULT_BATHYMETRY, foamProgress) : Infinity;
...
function TimelineStrip({ option, optionLabel, times }) {
  return (
    <div style={{ marginBottom: '20px' }}>
      <h3 style={{ margin: '0 0 8px 0', fontFamily: 'monospace', fontSize: '14px' }}>
        {optionLabel}
      </h3>
