...
export const NoDamping = () => (
  <div>
    ...
    >
      Energy pulse starts at row 0 (horizon) and propagates downward toward row 5 (shore). In deep
      water with no damping, energy maintains its magnitude as it travels. The pulse spreads
      vertically due to the blending algorithm (each row blends with the row above). Green = low
      energy, purple = high energy. Values show exact energy at each cell.
    </p>
    <ProgressionStrip
...
export const WithDamping = () => (
  <div>
    ...
    >
      Depth-based damping causes energy to decay faster in shallow water. The damping formula is:
      energy *= exp(-coefficient * dt / depth^exponent). Depth decreases linearly from 10m at
      horizon to 0.5m at shore. Notice how energy fades as it approaches the bottom rows (shallow
      water near shore). This prevents energy from "piling up" at the shoreline.
    </p>
    <ProgressionStrip
...
export const HighDamping = () => (
  <div>
    ...
    >
      Doubling the damping coefficient (0.2 vs 0.1) causes more aggressive energy decay. Energy
      dissipates before reaching the shore, which may be desirable to prevent visual artifacts at
      the shoreline. Compare with the previous story to see the difference. Too much damping can
      make waves feel weak; too little causes energy buildup at shore.
    </p>
    <ProgressionStrip
...
export const CompareAll = () => (
  <div>
    ...
    >
      All three damping configurations shown together for easy comparison. Look at how much energy
      remains at t=5s in each case. No damping: energy reaches shore at full strength. Coeff=0.1:
      energy reduced but still visible near shore. Coeff=0.2: energy mostly dissipated before
      reaching shore.
    </p>
    <ProgressionStrip snapshots={PROGRESSION_NO_DAMPING.snapshots} title="No Damping (coeff=0)" />
    <ProgressionStrip
      snapshots={PROGRESSION_WITH_DAMPING.snapshots}
...
export const DrainHole = () => (
  <div>
    ...
    >
      At t=1s, the center column (x=0.5) is completely drained, simulating a wave breaking. Compare
      each frame's center column to its neighbors - the center should be 0 or near-0 while neighbors
      retain energy. The drain persists as energy continues propagating, creating a vertical
      "stripe" where breaking removed all energy.
    </p>
    <ProgressionStrip
